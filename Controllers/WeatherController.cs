...
namespace WeatherAPI.Controllers
{
    public class WeatherController : Controller
    {
        ...
        public async Task<IActionResult> GetWeather(SearchWeatherVM searchInput)
        {
            ...
            var responseCurrent = await _httpClient.GetFromJsonAsync<CurrentModel>($"https://api.openweathermap.org/data/3.0/onecall?lat={responseGeo[0].Lat}&lon={responseGeo[0].Lon}&exclude=hourly,minutely,alerts&appid={_apiKey}");
            var responseForecast = await _httpClient.GetFromJsonAsync<WeatherForecastModel>($"https://api.openweathermap.org/data/3.0/onecall?lat={responseGeo[0].Lat}&lon={responseGeo[0].Lon}&exclude=hourly,minutely,alerts&appid={_apiKey}");
            ...
            searchWeatherVM.WeatherModelVM.CurrentWeather = responseCurrent;

            searchWeatherVM.WeatherModelVM.WeatherForecast = responseForecast;
